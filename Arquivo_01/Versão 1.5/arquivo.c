...
void insere_est(int id,cab * salva,FILE * arquivo,FILE * sq){
char nome[50];
  printf("Nome: ");
  setbuf(stdin,NULL);
  ler_nome(nome);
...
void excluir(cab * salva,FILE * arquivo,FILE * sq){
  int controle=0,achou=0;
  char dado[1000],nome[30];
  printf("Digite o nome que deseja excluir:");
  setbuf(stdin,NULL);
  ler_nome(nome);
  sq = fopen("temp.txt","w");
  arquivo = fopen("dados.txt","r");
  while(fgets(dado,sizeof(dado),arquivo) != NULL){
    if(strcmp(nome,dado) == 0){
...
sq = fopen("temp.txt","r");
arquivo = fopen("dados.txt","w");
while(fgets(dado,sizeof(dado),sq) != NULL){
